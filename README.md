...
The whole app builds to a single, self-contained HTML file — no server, no install:

- **▶ [Open it live](https://htmlpreview.github.io/?https://github.com/AltanS/aquaponics/blob/main/build/aquaponics-calculator.html)** — runs the latest checked-in build straight from this repo.
- **⤓ [Download the standalone file](https://github.com/AltanS/aquaponics/blob/main/build/aquaponics-calculator.html)** (`build/aquaponics-calculator.html`, ~120 kB) — open it in any browser, even offline. Logic, data and styles are all inlined; only the web fonts come from the network (they fall back to system fonts offline).

The artifact lives in [`build/`](build/) and is regenerated by `pnpm build:single`; the `pre-push` hook rebuilds it so the downloadable copy never drifts from source.
...
pnpm build:single # one self-contained HTML → build/aquaponics-calculator.html
...
A `pre-push` git hook runs the tests and refreshes the checked-in single-file
build (committing it if it changed, so re-run the push to include that commit). `pnpm dev` / `pnpm preview`
